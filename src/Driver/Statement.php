...
use Doctrine\DBAL\IBMIDB2PDO\Driver\Exception\CannotCopyStreamToStream;
use Doctrine\DBAL\IBMIDB2PDO\Driver\Exception\CannotCreateTemporaryFile;
...
//    private function bind(int $position, mixed &$variable, int $parameterType, int $dataType): void
    private function bind(string $parameter, mixed &$variable, int $parameterType, int $dataType): void
    {
        $this->parameters[$position] =& $variable;

//        if (! db2_bind_param($this->stmt, $position, '', $parameterType, $dataType)) {
        if (! $this->stmt->bindParam($parameter, '', $parameterType, $dataType)) {
